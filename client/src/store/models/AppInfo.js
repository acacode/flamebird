...
const AppInfo = {
	tasksMap: fieldArray(
		({ commands }) => reduce(commands, (map, command) => {
			if(!map[command.type]) { map[command.type] = [] }
			map[command.type].push(new Command(command))
			return map
		},{}),
		({ tasksMap }) => reduce(tasksMap, (commands, tasks) => [...commands, ...tasks.map(task => task.deserialize())],[])),
