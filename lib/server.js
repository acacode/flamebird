...
function start(taskfile, p, args) {
  ...
  app.post('/new-task', (req, res) => {
    ...
    const commandData = req.body.command
    const task = taskFile.createCommand(name, commandData, 'local', false)
    taskFile.addRawTaskData(task)
    storage.get('commands').push(task)
    res.send(task)
  })
