...
function createCommand(name, commandData, uniqueId, isNPM) {
  ...
  return {
    ...
    logs: [],
    isRun: false,
    index: name + uniqueId + commandCounter++,
    isNPM: !!isNPM,
  }
}
...
function addRawTaskData(task, rawTasks = COMMAND_PATHS) {
  const spaceChar = ' '
  let fixedTaskData = []
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    switch (word) {
      case 'npm': {
        fixNpmOrYarnTask(i, word, rawTasks, words, fixedTaskData)
        break
      }
      case 'yarn': {
        fixNpmOrYarnTask(i, word, rawTasks, words, fixedTaskData)
...
function updateCommands(commands) {
  ...
  // _.each(updatedCommands, command => {
  // rawTasks[command.name] = addRawTaskData(command, rawTasks)
  // })
  // rawTasks = null
  return updatedCommands
}

module.exports.addRawTaskData = addRawTaskData
