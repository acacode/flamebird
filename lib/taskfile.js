    ...
    try {
      ...
      _.forEach(packageJson.scripts, (command, taskName) => {
        if (!options.tasks.length || _.includes(options.tasks, taskName)) {
          allCommands.push(commands.createCommand(taskName, command, true))
        }
      })
    } catch (e) {
    ...
    try {
      ...
      _.each(_.compact(data.toString().split(/[\n\r]/g)), (line, i) => {
        if (line && line[0] !== '#') {
          ...
          if (!options.tasks || _.includes(options.tasks, name)) {
            allCommands.push(commands.createCommand(name, task, false))
          }
        }
      })
    } catch (e) {
...
function updateCommands(commands) {
  ...
  if (options.ignorePms) {
    ...
    _.each(commands, command => {
      command.rawTask = command.isNPM
        ? `${pms[options.useAnotherPm] || options.useAnotherPm} ${command.name}`
        : command.task
    })
  }
