...
function load(filename, args) {
  ...
  return storage.set(
    'commands',
    updateCommands(allCommands, args.absolutePaths)
  )
}

function updateCommand(command, rawTasks, absolutePaths) {
  if (!absolutePaths) return (command.rawTask = command.task)
  const spaceChar = ' '
  let fixedTaskData = []
...
function updateCommands(commands, absolutePaths) {
  if (LIB_PATHS === null) createLibPaths()
  let updatedCommands = _.sortBy(
    commands,
    c => _.includes(c.task, 'yarn') || _.includes(c.task, 'npm')
  )
  _.reduce(
    updatedCommands,
    (patchedCommands, command) => {
      command.task = _.replace(command.task, 'cross-env ', '')
      patchedCommands[command.name] = updateCommand(
        command,
        patchedCommands,
        absolutePaths
      )
