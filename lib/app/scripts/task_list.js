window.TaskList = (function() {
  ...
  function onChangeTab(name) {
    ...
    setTimeout(function() {
      ...
      if (activeTaskByTab[name]) {
        el(`#${activeTaskByTab[name]}`).click()
      } else {
  ...
  function updateTask(id, isRun, isActive, isLaunching, isStopping) {
    ...
    if (task.isActive) {
      ...
      if (prevActiveTask) {
        const prevTaskId = activeTaskByTab[task.type]
  ...
  function getAllFromActiveTab(isRun) {
    ...
    if (isRun) {
      _.filter(tasks, { isRun: true })
    }
    return tasks
  }

  const getActive = () => getTask(activeTaskByTab[Tabs.getActive().name])
  ...
  return function(taskListContainer, tasks) {
    ...
    return {
      ...
      setActive: setActive,
      getActive: getActive,
      getActiveTab: () => Tabs.getActive().name,
