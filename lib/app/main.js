...
function displayInfo(isNPM) {
  ...
  if (commands.length) {
	...
	if(filteredCommands.length){
		...
		$('#' + (isNPM ? activeNPMCommand || filteredCommands[0].name : activeProcfileCommand || filteredCommands[0].name)).trigger('click')
	}
  }
}

function runAllTasks(e) { // eslint-disable-line
  var filteredCommands = filterCommands($('.tab#packageJson').hasClass('active'),false)
  if(filteredCommands.length){
...
function stopAllTasks(e) { // eslint-disable-line
  var filteredCommands = filterCommands($('.tab#packageJson').hasClass('active'),true)
  if(filteredCommands.length){
    _.forEach(filteredCommands,function(c){
...
function findCommandByName(name, isNPM) {
  return _.find(commands, function(c) {
    return c.name === name && c.isNPM === !!isNPM
  })
}
...
activeNPMCommand = null
activeProcfileCommand = null
