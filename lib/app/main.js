...
function triggerScrollWatcher(e) { // eslint-disable-line
  WebLogger.scrollTo('bottom', true)
...
function setActiveCommand(command, isStartRunning) {
  ...
  if (previousActiveTask) {
    ...
    if (id !== command.name) {
      findCommandByName(previousActiveTask.getAttribute('id')).isActive = false
...
function openTask(name) {  // eslint-disable-line
  ...
  logger.clear(true)
  axios.get('/logs/' + command.name).then(function(response) {
    logger.updateDescription(command.task)
    logger.updateEnvs(command.envs)
    _.forEach(response.data, function(log, index) {
      setTimeout(function() {
        logger.push(log)
        // if (index === response.data.length - 1) {
        WebLogger.scrollTo('bottom')
        // }
      }, 0)
    })
  })
...
$(document).ready(function() {
  ...
  new WebSocket('ws://' + location.host).onmessage = function(message) {
    ...
    if (data.name) {
      let command = findCommandByName(data.name, data.isNPM)
      var taskButton = document.getElementById(data.name)
      command.isRun = data.isRun
      if (data.log) {
        if (command.isActive) {
          setTimeout(function() {
            logger.push(data.log)
            if (watchTaskLogsScrollTop) WebLogger.scrollTo('bottom')
          }, 0)
        }
      }
      command.isStartRunning = false
      taskButton.classList.remove('clicked', 'stopping')
      taskButton.classList[data.isRun ? 'add' : 'remove']('running')
    }
  }
})
