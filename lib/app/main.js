/* global $, axios, _ , FileLoader, WebLogger, HotKeys, TaskList, createSpan */
...
window.global = (function() {
 ...
 axios.post('/stop/' + activeTask.name).then(function(){
  ...
  function updateHotkeys() {
    ...
    document
      .querySelector('.main-button.hot-keys')
      .classList[hotKeysEnabled ? 'add' : 'remove']('active')
  ...
  $(document).ready(function() {
    ...
    axios.get('/info').then(function(response) {
      ...
      ws.onmessage = function(message) {
        ...
        if (data.name) {
          var activeCommand = taskList.getActive()
          var isActive = data.name === activeCommand.name
          taskList.updateTask(
            data.name,
            data.isRun,
            isActive,
